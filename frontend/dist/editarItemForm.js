...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!itemId) {
        document.body.innerHTML = '<h1>ID do item não fornecido.</h1>';
        return;
    }
    ...
    try {
        ...
        document.getElementById('form-container').innerHTML = `<p style="color: red;">Não foi possível carregar os dados do item.</p>`;
    }
    form?.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            if (mensagemEl)
                mensagemEl.textContent = 'Erro de conexão.';
        }
    });
});
